...
const MemoryInputWidget = ({ onSave }: MemoryInputWidgetProps) => {
  ...
  if (status === 'saved') {
    return (
      <div className="h-full bg-warm-white rounded-3xl shadow-xl flex flex-col items-center justify-center p-8 animate-fade-in text-center">
        <div className="h-24 w-24 rounded-full bg-sage/20 flex items-center justify-center animate-bounce-short mb-6">
          <CheckCircle className="w-12 h-12 text-sage" />
        </div>
        <h2 className="text-3xl font-bold text-sage mb-2">Saved to Vault</h2>
        <p className="text-warm-grey/60 mb-8">Memory secured safely.</p>

        <button
          type="button"
          onClick={handleReset}
          className="h-16 px-8 bg-sand text-warm-grey hover:bg-sage hover:text-white rounded-2xl text-lg font-bold transition-all flex items-center space-x-2"
        >
  ...
  if (status === 'processing' || status === 'review') {
    return (
      <div className="h-full bg-warm-white rounded-3xl shadow-xl flex flex-col items-center justify-center p-8 animate-fade-in text-center relative overflow-hidden">
        <div className="absolute inset-0 bg-sage/5 animate-pulse"></div>

        <Loader2 className="w-20 h-20 text-sage animate-spin mb-6 relative z-10" />
        <h2 className="text-3xl font-bold text-sage mb-2 relative z-10">Tidying Up...</h2>
        <p className="text-warm-grey/60 text-lg relative z-10">Organizing your memory into the vault.</p>
      </div>
    );
  }
  ...
  if (showTextInput && !isListening) {
    return (
      <div className="h-full bg-warm-white rounded-3xl shadow-xl flex flex-col overflow-hidden relative transition-all">
        <div className="absolute top-4 right-4 z-30">
          <button
            type="button"
            onClick={() => setShowTextInput(false)}
            className="p-3 bg-sand rounded-xl text-warm-grey hover:bg-sand/80"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        <div className="flex-1 p-6 flex flex-col space-y-4 animate-fade-in">
          ...
          <textarea
            ...
            className="flex-1 w-full bg-sand rounded-2xl p-6 text-xl text-warm-grey placeholder-warm-grey/40 resize-none focus:outline-none focus:ring-4 focus:ring-sage/20"
          />
          <button
            type="button"
            onClick={() => handleStopAndTidy()}
            disabled={!inputText.trim()}
            className="w-full h-20 bg-rust text-white rounded-2xl text-xl font-bold hover:bg-opacity-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Tidy & Save
          </button>
        </div>
      ...
      {!isListening && (
        <div className="absolute top-4 right-4 z-30">
          <button
            ...
            className="p-3 bg-sand text-warm-grey rounded-xl hover:bg-sand/80 transition-colors"
          >
            <Keyboard className="w-6 h-6" />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-8 relative">
        {isListening && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden">
            <div className="w-64 h-64 bg-rust/5 rounded-full animate-ping opacity-75 absolute"></div>
            <div className="w-48 h-48 bg-rust/10 rounded-full animate-ping opacity-75 animation-delay-300 absolute"></div>
          </div>
        )}

        <div className="text-center z-10 space-y-2">
          <h2 className="text-3xl font-bold text-sage">{isListening ? "I'm Listening..." : 'Capture a Memory'}</h2>
          <p className="text-warm-grey/60 text-lg transition-all duration-300">
        ...
        <button
          ...
          className={`
            ...
            ${isListening ? 'bg-rust scale-110 shadow-rust/40' : 'bg-sage hover:bg-opacity-90 hover:scale-105 shadow-sage/30'}
          `}
        >
          {isListening ? (
            <Mic className="w-20 h-20 text-white animate-bounce-short" />
          ) : (
        ...
        <div className="h-8">
          {isListening && silenceSeconds > 0.5 && (
            <span className="text-rust font-medium text-sm animate-pulse">Auto-saving in {Math.ceil(4 - silenceSeconds)}s...</span>
          )}
        </div>

        {activeError && (
          <div className="absolute bottom-12 bg-warning/80 backdrop-blur-sm text-rust px-6 py-3 rounded-2xl flex items-center space-x-3 animate-fade-in z-30 shadow-sm border border-warning max-w-[80%]">
